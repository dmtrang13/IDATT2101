...
void quicksort_3way_core(vector<int> &A, int l, int r) {
    if (l >= r) return;
    int pivot = A[(l + r) / 2];        // median-of-three would be even safer
    int lt = l, i = l, gt = r;         // [l..lt-1] < pivot, [lt..i-1] == pivot, [gt+1..r] > pivot
    while (i <= gt) {
        if (A[i] < pivot)      swap(A[lt++], A[i++]);
        else if (A[i] > pivot) swap(A[i], A[gt--]);
        else                   ++i;
    }
    quicksort_3way_core(A, l, lt - 1);
    quicksort_3way_core(A, gt + 1, r);
}

void quicksort_3way(vector<int> &A) {
    if (!A.empty()) quicksort_3way_core(A, 0, (int)A.size() - 1);
}
...
int main() {
    ...
    long long t_unique1 = bench_ns(unique, [&] (vector<int> &A){quicksort_3way(A); }, BenchMode::Copy);
    cout << "Dijkstra 3 way partition quicksort, med unike tall: " << t_unique1 << "ns\n";
    (void)bench_ns(unique, [](vector<int> &A){ quicksort_3way(A); }, BenchMode::InPlace);
    cout << "Sum etter: " << checksum(unique) << "\n";

    // Resorter faktisk lista (InPlace)
    long long t_unique2 = bench_ns(unique, [&] (vector<int> &A){quicksort_3way(A); }, BenchMode::InPlace);
    ...
    long long t_dups = bench_ns(dups, [&] (vector<int> &A){quicksort_3way(A); });
    cout << "Dijkstra 3 way partition quicksort, med mange duplikater: " << t_dups << "ns\n";
    (void)bench_ns(dups, [](vector<int> &A){ quicksort_3way(A); }, BenchMode::InPlace);
